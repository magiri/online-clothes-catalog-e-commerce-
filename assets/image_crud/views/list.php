...
<div id='ajax-list'>
	...
	<?php foreach($photos as $photo_num => $photo){?>
            ...
            <li id="photos_<?php echo $photo->$primary_key; ?>" class="col-md-2">
				<div class='photo-box'>
					<a href='<?php echo $photo->image_url?>' <?php if (isset($photo->title)) {echo 'title="'.str_replace('"',"&quot;",$photo->title).'" ';}?>target='_blank' class="color-box" rel="color-box" tabindex="-1"><img src='<?php echo $photo->image_url?>' width='90' height='60' class='basic-image img-rounded' /></a>
					...
					<div class="clear"></div>
                                        <?php if ($prod_id != 0): ?>
                                        <a href="<?php echo site_url('products/admin/select_image/'.$prod_id.'/'.$photo->$primary_key); ?>" > <button> Select Image</button></a>
                                        <?php endif; ?>
				</div>
			</li>
